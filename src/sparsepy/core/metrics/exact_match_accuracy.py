...
class ExactMatchAccuracyMetric(Metric):
    ...
    def compute(self, m: Model, last_batch: torch.Tensor, labels: torch.Tensor, training: bool = True):
        """
        Computes the exact match accuracy of a model for a given batch of inputs.

        Args:
            m: Model to evaluate.
            last_batch: the model input for the current step (as a Tensor)
            labels: the model output for the current step (as a Tensor)
            training: boolean - whether the model is training (store codes) or evaluating (determine exact match accuracy using stored codes)
                ...
                if not training:
                    ...
                    for layer_index in range(len(layer_outputs)):
                        ...
                        for mac_index in range(layer_outputs[layer_index].shape[0]):
                            ...
                            exact_match = torch.allclose(self.stored_inputs[image_str][layer_index][mac_index], layer_outputs[layer_index][mac_index], atol=1e-5)
                            # if so, increment hits
                            if exact_match:
                                hits += 1
        
        # return final exact match accuracy as a fraction
