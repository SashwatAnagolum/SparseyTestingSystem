...
class DataStorer:
    ...
    def save_model(self, experiment: str, m: Model, model_config: dict):
        ...
        if DataStorer.wandb_config["save_models"]:
            ...
            model_name = model_config.get("model_name", instance_name)
            # model description: string description to log into W&B model registry
            model_description = model_config.get(
            ...
            # add the state dict
            model_artifact.add_file(local_path=model_path, name="model.pt")

            # add the model config file (required for rehydration)
            with model_artifact.new_file("network.yaml") as f:
